  ...
  reducers: {
    addSession: (
      ...
      action: PayloadAction<{
        ...
        preloadedState?: Partial<BaseSessionState>;
      }>,
    ) => {
    ...
    removeSession: (state, action: PayloadAction<{ sessionId: string }>) => {
      ...
      if (action.payload.sessionId === state.currentSessionId) {
        state.currentSessionId =
          state.sessionEntities.ids.length > 0 ? state.sessionEntities.ids[0].toString() : null;
      }
    ...
    setCurrentSessionId: (state, action: PayloadAction<{ sessionId: string | null }>) => {
      if (
        ...
        state.sessionEntities.ids.includes(action.payload.sessionId)
      ) {
        state.currentSessionId = action.payload.sessionId;
      } else {
        console.error(
          `tried to change to session ${action.payload.sessionId}, but it doesn't exist!`,
        );
      }
    },
  },
...
export async function setInitialSessionManagerState() {
  ...
  for (const currentSession of currentSessions) {
    const preloadedSessionState: BaseSessionState | undefined =
      preloadedSessionStateResults[currentSession.id];
    store.dispatch(
      addSession({
...
export const { addSession, removeSession, sessionsInitialized, setCurrentSessionId } =
